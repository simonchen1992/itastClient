...
def exec_vcasCommand(cmd, caseID):
  response = []
  exec 'response = %s' % cmd
      ...
      for dutID in deviceID:
        ...
        for pos in robot.points(1, 40, device_orientation):
          ...
          while attempt < 7:
            ...
            exec_vcasCommand("itast.client.sdkPrepareTransaction(tx, dutID)", caseID)  # Prepare transaction
            ...
            print txverdict
            exec_vcasCommand("itast.client.sdkGetDebugLog(tx, dutID)")  # Getdebuglog from last transaction
